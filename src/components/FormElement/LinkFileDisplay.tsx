...
const LinkFileDisplay = ({
  ...
  const handleFileCheck = async () => {
    ...
    if (typeof filepath === 'string') {
      url = await getPresignedImageUrl(filepath, undefined, undefined, false);
  ...
  const checkAndRemove = (
    ...
    if (
      ...
      !Object.values(EnumFileType).includes(
  ...
  return (
    <div
      className={`${
        isBlack
          ? ''
          : 'bg-authBG/40 p-4 border border-solid border-authBG/80 rounded'
